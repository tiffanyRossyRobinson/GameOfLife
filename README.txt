...
This will be accomplished in the following way: 
	...
	*The Process
		...
		Once on the app, the user will have the opportunity to pick their iteration value, the number of rows, the number of columns, and create their life space. 

		When the life space is created, they are able to bring the dead(cells) to life. After they have brought the cells to life, they can hit the start button and see who will survive through the interation period selected.

	*Determination Method
		
		When the user presses the start button, the app will collect the table cells that were create using the someObject.classes element that was stored during the life space build. This is going to tell the application what spaces it needs to look at. 
		...
		For each iteration, a blank needToUpdate array will be created to keep track of which cells need to either become ALIVE or DEAD.

		For each space (someObject.classes array), the application will pull off the row and col values and determine which cells surround this element and create a neighborsArray. 

		Now that it know who the neighbors are, it will go through and figure out which ones are ALIVE. Running through the determination rules that were defined earlier, it will determine if this cell needs to be changed to ALIVE or DEAD. If it needs to change states, it will push this cell value into the needToUpdate array.
